...
- (void)viewDidLoad {
    ...
    self.localView.layer.borderWidth = 1.f;
    self.localView.layer.borderColor = [UIColor redColor].CGColor;
    
    self.defoultRemoveFrame = self.remoteView.frame;
    ...
    self.defoultTransformLocalView = self.localView.transform;
    self.defoultTransformRemoveView = self.remoteView.transform;
...
- (IBAction)audioButtonPressed:(id)sender {
    ...
    if (self.isAudioMute) {
       // [self.client disconnect];
      //  [self.client connectToRoomWithIdTest:self.roomName options:nil];
       // [self.client unmuteAudioIn];
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
            ...
            [audioSession setActive:NO withOptions:nil error:nil];
...
- (void)appClient:(ARDAppClient *)client didError:(NSError *)error {
    //[NSString stringWithFormat:@"Room is full. Enter the room name again" ]
    // message:[NSString stringWithFormat:@"%@", error]

    UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:nil
                                                        message:[NSString stringWithFormat:@"%@", error.userInfo.allValues.firstObject]
                                                       delegate:self
...
- (void)videoView:(RTCEAGLVideoView *)videoView didChangeVideoSize:(CGSize)size {
    ...
    if(self.oldOrientation == UIDeviceOrientationUnknown) {
        ...
        if(self.oldOrientation != orientation) {
            if(orientation == UIDeviceOrientationPortrait) {
                self.remoteView.transform = self.defoultTransformRemoveView ;
                self.remoteView.frame = self.defoultRemoveFrame;
            }
        if(orientation == UIDeviceOrientationPortrait) {
            self.remoteView.transform = self.defoultTransformRemoveView ;
            CGFloat radians = atan2f(self.remoteView.transform.b, self.remoteView.transform.a);
            CGFloat degrees = radians * (180 / M_PI);
            CGAffineTransform transform = CGAffineTransformMakeRotation((90 + degrees) * M_PI/180);
            self.remoteView.transform = transform;
            
            
            CGFloat containerWidth = self.remoteView.frame.size.width ;
            
            CGRect frameRect = self.view.frame;
            frameRect.size.width = self.view.frame.size.height;
            frameRect.size.height = containerWidth;
            self.remoteView.frame = frameRect;
        }
        }
       
      
            self.flagRotate = YES;
            if(self.remoteVideoSize.height < self.remoteVideoSize.width)
    ...
    if(orientation == UIDeviceOrientationLandscapeLeft ){
        self.localView.transform = self.defoultTransformLocalView ;
        CGFloat radians = atan2f(self.localView.transform.b, self.localView.transform.a);
        CGFloat degrees = radians * (180 / M_PI);
        CGAffineTransform transform = CGAffineTransformMakeRotation((90 + degrees) * M_PI/180);
        self.localView.transform = transform;
        self.localViewWidthConstraint.constant = 140;
        self.localViewHeightConstraint.constant = 108;
   
        self.localViewRightConstraint.constant =0;
    ...
    if(orientation == UIDeviceOrientationPortrait){
     ...
     self.localView.transform = self.defoultTransformLocalView ;
     self.localViewWidthConstraint.constant = 108;
     self.localViewHeightConstraint.constant = 140;
        self.localViewRightConstraint.constant =20;
    }
...
/*
 ...
 [self.localViewWidthConstraint setConstant:videoFrame.size.width + 150];
 [self.localViewHeightConstraint setConstant:videoFrame.size.height + 200];
 if (self.remoteVideoTrack) {
 [self.localViewBottomConstraint setConstant:0.0f]; //bottom right corner
 [self.localViewRightConstraint setConstant:0.0f];
 } else {
 [self.localViewBottomConstraint setConstant:0.0f]; //center
 [self.localViewRightConstraint setConstant:containerWidth/2.0f - videoFrame.size.width + 150/2.0f]; //center
 }
 } else if (videoView == self.remoteView) {
 //Resize Remote View
 self.remoteVideoSize = size;
 CGSize aspectRatio =  defaultAspectRatio ;
 CGRect videoRect = self.view.bounds;
 CGRect videoFrame = AVMakeRectWithAspectRatioInsideRect(aspectRatio, videoRect);
 if (self.isZoom) {
 //Set Aspect Fill
 CGFloat scale = MAX(containerWidth/videoFrame.size.width + 150, videoFrame.size.height + 200);
 videoFrame.size.width *= scale;
 videoFrame.size.height *= scale;
 }
 
 [self.remoteViewTopConstraint setConstant:containerHeight/2.0f - videoFrame.size.height + 200/2.0f];
 [self.remoteViewBottomConstraint setConstant:containerHeight/2.0f - videoFrame.size.height + 200/2.0f];
 [self.remoteViewLeftConstraint setConstant:containerWidth/2.0f -  videoFrame.size.width + 150/2.0f]; //center
 [self.remoteViewRightConstraint setConstant:containerWidth/2.0f -  videoFrame.size.width + 150/2.0f]; //center
